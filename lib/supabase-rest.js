...
export async function persistStories(stories, sources) {
  ...
  for (const source of sources) {
    await request(`sources?id=eq.${encodeURIComponent(source.id)}`, {
      method: 'POST',
      body: JSON.stringify({
        id: source.id,
        name: source.name,
  ...
  for (const story of stories) {
    const sourceRows = await request(
      `sources?id=eq.${encodeURIComponent(story.sourceId)}&select=id`
    );
    ...
    const rows = await request(
      `stories?url=eq.${encodeURIComponent(story.url)}`,
      {
        method: 'POST',
    ...
    for (const entity of story.entities || []) {
      ...
      await request(
        `story_entities?story_id=eq.${encodeURIComponent(saved.id)}&name=eq.${encodeURIComponent(name)}`,
        {
          method: 'POST',
          body: JSON.stringify({
            story_id: saved.id,
...
export async function getRecentStories(limit = 200) {
  ...
  const rows =
    (await request(
      `stories?select=id,title,description,summary,image_url,category,published_at,momentum_score,importance_score,market_relevance_score,africa_relevance,sentiment,url&order=published_at.desc&limit=${Math.min(limit, 500)}`
    )) || [];
...
export async function getStory(id) {
  if (!configured()) return null;
  const rows = await request(
    `stories?id=eq.${encodeURIComponent(id)}&select=*,sources(name,url)&limit=1`
  );
  const story = rows?.[0];
  if (!story) return null;

  const entities = await request(
    `story_entities?story_id=eq.${encodeURIComponent(id)}&select=name,entity_type`
  );
  const snapshots = await request(
    `story_snapshots?story_id=eq.${encodeURIComponent(id)}&select=momentum_score,importance_score,captured_at&order=captured_at.asc`
  );
  const related = await request(
    `stories?category=eq.${encodeURIComponent(story.category)}&id=neq.${encodeURIComponent(id)}&select=id,title,category,published_at,momentum_score,url,image_url&order=published_at.desc&limit=5`
  );
...
export async function getTopic(slug) {
  ...
  if (isCategory) {
    stories = await request(
      `stories?category=ilike.${encodeURIComponent(topic)}&select=id,title,category,published_at,momentum_score,importance_score,market_relevance_score,africa_relevance,sentiment,url,image_url&order=published_at.desc&limit=50`
    );
  } else {
