...
import { prepareImageUrl } from './image-pipeline';
import { resolveOriginalImage } from './image-resolver';
...
function firstImage(block) {
  ...
  return image ? prepareImageUrl(clean(image)) : '';
}
function extractItems(xml) {
  return [...xml.matchAll(/<(item|entry)(?:\s[^>]*)?>([\s\S]*?)<\/(item|entry)>/gi)].map((match) => {
    const block = match[2];
    const linkHref = block.match(/<link[^>]+href=["']([^"']+)["'][^>]*>/i)?.[1];
    return { title: firstTag(block, 'title'), description: firstTag(block, 'description') || firstTag(block, 'summary') || firstTag(block, 'content'), url: linkHref || firstTag(block, 'link') || firstTag(block, 'guid'), imageUrl: firstImage(block), publishedAt: firstTag(block, 'pubDate') || firstTag(block, 'published') || firstTag(block, 'updated') };
  });
}
async function resolveImages(items) {
  ...
  async function worker() {
    while (cursor < output.length) {
      ...
      output[index] = { ...item, imageUrl: await resolveOriginalImage(item.url, item.imageUrl) };
    }
  }
  await Promise.all(Array.from({ length: Math.min(IMAGE_RESOLVE_CONCURRENCY, output.length) }, worker));
  return output;
}
async function ingestOne(source) {
  ...
  try {
    const response = await fetch(source.url, { headers: { 'user-agent': 'InferaBot/1.0', accept: 'application/rss+xml, application/atom+xml, application/xml, text/xml, */*' }, cache: 'no-store', signal: controller.signal });
    ...
    return { source, items: await resolveImages(items), error: null };
  } catch (error) {
...
export async function ingestSources() { return Promise.all(NEWS_SOURCES.map(ingestOne)); }
